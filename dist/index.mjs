...
// src/parser.ts
import { ErrorList as ErrorList2 } from "@scratch-fuse/utility";
...
var Parser = class {
  ...
  skipComments() {
    while (this.current < this.tokens.length && (this.tokens[this.current].type === "Comment" /* Comment */ || this.tokens[this.current].type === "Eol" /* Eol */)) {
      this.current++;
    }
  }
  ...
  synchronize() {
    ...
    while (!this.isEof()) {
      if (this.current > 0 && this.previous().type === "Eol" /* Eol */) return;
      if (!this.isEof()) {
...
function toSource(node, indent = 2, semi = false) {
  ...
  function formatNamespaceValue(value, level) {
